...
# ---- load-data -------------------------------------------------------------
# covid data
ds_covid <- readr::read_csv("./data-public/derived/covid-2020-05-31.csv")
ds_covid %>% glimpse()
...
d_reprex_timeline <- d_reprex %>%
  ...
  mutate(
    ...
    ,date_of_1case = min(date_of_1case1, na.rm = T)
...
g2 <-
  ...
  geom_line()+
    # ggrepel::geom_text_repel(aes(label = country_code), data = d1 %>% filter(date == as_date(Sys.Date())))+
...
g3dev <-
  ...
  geom_text(aes(label = country_code2, x = date_of_1death), hjust = -1, size =3, color = "grey50")
